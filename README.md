<!-- ═══════════════════════════════════════════════════════════════ -->
<!--   BORWORNPAKK — GitHub Profile README                         -->
<!--   วิธีใช้: วางไฟล์นี้ใน repo ชื่อ "Borwornpakk/Borwornpakk"  -->
<!--   แล้ว GitHub จะแสดงหน้า Profile อัตโนมัติ                   -->
<!-- ═══════════════════════════════════════════════════════════════ -->

<div align="center">

<!-- ── Animated Typing Header ── -->

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=202836&center=true&vCenter=true&width=435&lines=Technical+Service+Engineer;IoT+%26+Structural+Innovation;Material+%26+Construction+Chemical)](https://git.io/typing-svg)

<!-- ── Name & Tagline ── -->
# 👋 Suwapitch Borwornpakvuddhiparami

Civil & Materials Engineer (8+ Years) | Building Maintenance & Graduate Research<Br>Expert in technical services, repair material specification, and application-focused problem solving.

<br>

<!-- ── Quick Contact Badges ── -->
[![Email](https://img.shields.io/badge/Email-suwapitch.borworn%40outlook.com-0078D4?style=flat-square&logo=microsoft-outlook&logoColor=white)](mailto:suwapitch.borworn@outlook.com)
[![Location](https://img.shields.io/badge/Based_in-Nonthaburi%2C_Thailand-00B894?style=flat-square&logo=google-maps&logoColor=white)](https://maps.google.com/?q=Nonthaburi,Thailand)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-SuwapitchBorwornpakvuddhiparami-0A66C2?style=flat-square&logo=linkedin&logoColor=black)](https://linkedin.com/in/suwapitch-borwornpakvuddhiparami)


</div>

---

## 🧠 About Me

```python
engineer = {
    "name"      : "Suwapitch Borwornpakvuddhiparami",
    "role"      : "Technical Service / Application Engineer",
    "experience": "8+ years in structural assessment & material specification",
    "research"  : ["Fraunhofer IWU, Germany (2023–2024)",
                   "Kasetsart University, Thailand (MSc 2013–2016)"],
    "currently" : "Building IoT monitoring systems & ML models for building materials",
    "open_to"   : "Technical Engineer | Application Engineer | Field Engineer roles",
}
```

I turn complex structural problems into engineered solutions — from diagnosing building failures on-site at EGAT, to validating polymer composites with ANSYS multiphysics at Fraunhofer IWU in Dresden. Now expanding into **predictive maintenance** by combining IoT sensor pipelines with machine learning.

---

## 🚀 Featured Projects

### 🌐 IoT-Based Joint Movement Monitoring System
> Real-time building facade health tracking via sensor-to-dashboard pipeline

```
Sensor → MQTT Broker → Node-RED → Data Processing → Dashboard
  📡           🔀            ⚙️             📊              📺
```

- Captures **thermal expansion & joint movement** data in real-time
- Cross-references against silicone sealant elongation limits for failure prediction
- Supports **predictive maintenance** decisions for building facades

**Tech Stack:**
![Node-RED](https://img.shields.io/badge/Node--RED-8F0000?style=flat-square&logo=node-red&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-Protocol-660066?style=flat-square&logo=eclipse-mosquitto&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

---

### 🤖 ML Model — Silicone Sealant Degradation Prediction
> Predicting long-term elastomeric sealant performance under UV & thermal cycling

- Python-based predictive model integrating **material science + data analytics**
- Simulates degradation curves for substrate compatibility analysis
- Supports technical troubleshooting and product recommendation workflows

**Tech Stack:**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)

---

### 🔬 Fraunhofer IWU Research — Printed Wire-Reinforced Polymer Composites
> Thermo-mechanical characterization + ANSYS multiphysics simulation validation

- Investigated **FFF-printed wire-reinforced polymer composites** under intrinsic heating
- Conducted three-point bending tests with simultaneous **infrared thermography**
- Applied **DSC** to determine Tg of PC and PETG matrices
- Validated against coupled **ANSYS (thermal steady-state + structural)** simulation

**Tech Stack:**
![ANSYS](https://img.shields.io/badge/ANSYS-Multiphysics-FFB71B?style=flat-square&logo=ansys&logoColor=black)
![CAD](https://img.shields.io/badge/SolidWorks-E51050?style=flat-square&logo=autodesk&logoColor=white)

---

## 🛠️ Technical Skill Stack

<div align="center">

| Domain | Tools & Technologies |
|:---|:---|
| 🔬 **Engineering Simulation** | ANSYS Mechanical · CFX · FSI (Fluid-Structure Interaction) |
| 📡 **IoT & Smart Systems** | Node-RED · MQTT · Raspberry Pi · Sensor Integration |
| 🐍 **Data & ML** | Python · Pandas · scikit-learn · Data Analytics |
| 🏗️ **CAD & BIM** | AutoCAD · Autodesk Revit · SolidWorks |
| 🔥 **3D Printing** | Fused Filament Fabrication · G-Code · E3D ToolChanger · Superslicer
| 🧱 **Materials Engineering** | Silicone Sealants · Waterproofing · Structural Coatings · DSC · Polymer Composite |
| 🔍 **Core Methods** | Root Cause Analysis · Structural Assessment · Contractor Management |

</div>

---

## 📊 GitHub Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Borwornpakk&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00B894&icon_color=00B894&text_color=ffffff)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Borwornpakk&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00B894&text_color=ffffff)

</div>

---

## 💼 Career Timeline

```
2006–2010  B.Eng Civil Engineering          │ Kasetsart University, Thailand
2011–2017  Civil Engineer (Technical Svc)   │ EGAT — Building assessment & repair spec
2013–2016  M.Eng Structural Technology      │ Kasetsart University (part-time)
              └─ FSI simulation of wind turbine blades · Validated vs NREL data
2021–2024  M.Sc Advanced Manufacturing      │ TU Chemnitz, Germany
2023–2024  Graduate Researcher              │ Fraunhofer IWU, Dresden Germany
              └─ ANSYS multiphysics · Infrared thermography · DSC characterization
2025–Now   Independent Technical Dev        │ IoT monitoring · ML for sealants
```

---

## 🎓 Education & Certifications

| Degree | Institution | Year |
|:---|:---|:---:|
| M.Sc. Advanced Manufacturing | Technische Universität Chemnitz, Germany | 2024 |
| M.Eng. Structural Technology | Kasetsart University, Thailand | 2016 |
| B.Eng. Civil Engineering | Kasetsart University, Thailand | 2010 |
| PMP Certification Exam Prep Course 35 PDU Contact Hours/PDU | Technical Institute of America | 2025 |
| Design of Steel Structures | Engineering Institute of Thailand | — |

---

## 🌍 Languages

![Thai](https://img.shields.io/badge/Thai-Native-DC143C?style=flat-square)
![English](https://img.shields.io/badge/English-Professional-0078D4?style=flat-square)
![German](https://img.shields.io/badge/German-Conversational-black?style=flat-square&logo=germany)

---

<div align="center">

**🤝 Open to Technical Service · Application Engineer · Field Engineering roles**

*Feel free to reach out for collaboration or technical consultation!*

[![Email Me](https://img.shields.io/badge/📩_Email_Me-suwapitch.borworn%40outlook.com-00B894?style=for-the-badge)](mailto:suwapitch.borworn@outlook.com)

</div>

